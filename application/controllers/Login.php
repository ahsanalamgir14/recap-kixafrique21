...

{
    ...
    {
        // Receive webhook request from Laravel project
        $email = $this->input->post('email');
        $password = $this->input->post('password');
    
        // Authenticate user in CodeIgniter project
        $credential = array('email' => $email, 'password' => sha1($password), 'status' => 1);
        $query = $this->db->get_where('users', $credential);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->input->set_cookie('ci_session', $this->session->userdata('session_id'), 86400);
    
            // Return session data in JSON response
            $response = array('success' => true, 'session_data' => $this->session->userdata());
            echo json_encode($response);
    
            // Redirect to home page
            // header('Refresh:0;url=http://localhost:8080/home'); // Commented out to avoid redirecting before JSON response is sent
            exit;
        } else {
    ...
    public function register_via_webhook()
    {
        // Get the incoming data from Laravel
        $data = [
            'first_name' => $this->input->post('first_name'),
            'last_name' => $this->input->post('last_name'),
            'email' => $this->input->post('email'),
            'password' => sha1($this->input->post('password')), // Hash using sha1
            'status' => $this->input->post('status'), // Status from Laravel
            'role_id' => $this->input->post('role_id'), // Role from Laravel
        ...
        if ($validity === 'unverified_user' || $validity == true) {
            if ($validity === true) {
                ...
                // Update the verification code for unverified users
                $this->user_model->register_user_update_code($data);
            }

            // Optionally send verification email if required
            if (get_settings('student_email_verification') == 'enable') {
    ...
    // private function trigger_laravel_webhook($data)
    // {
